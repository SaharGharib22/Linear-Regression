...
# Train the Ordinary Least Squares regression model
ols_reg = LinearRegression()
ols_reg.fit(X_train_scaled, y_train)

# Make predictions
y_pred_train = ols_reg.predict(X_train_scaled)
y_pred_test = ols_reg.predict(X_test_scaled)
...
plt.plot(X_test, y_pred_test, color='red', linewidth=2, label='OLS Regression Line')
plt.xlabel('x')
plt.ylabel('y')
