...
multi_task_lasso.fit(X_train_scaled, Y_train)

# Make predictions
Y_pred_train = multi_task_lasso.predict(X_train_scaled)
Y_pred_test = multi_task_lasso.predict(X_test_scaled)
...
for i, target in enumerate(['y', 'y1', 'y2']):
    ...
    plt.plot(X_train, Y_pred_train[:, i], color='red', linewidth=2, label='Multi-task Lasso fit')
    plt.title(f'Training Data and Multi-task Lasso Regression Fit for {target}')
    plt.xlabel('X')
    plt.ylabel(target)
    plt.legend()
...
for i, target in enumerate(['y', 'y1', 'y2']):
    ...
    plt.plot(X_test, Y_pred_test[:, i], color='red', linewidth=2, label='Multi-task Lasso fit')
    plt.title(f'Testing Data and Multi-task Lasso Regression Fit for {target}')
    plt.xlabel('X')
    plt.ylabel(target)
    plt.legend()
...
print("Model coefficients:", multi_task_lasso.coef_)
print("Model intercept:", multi_task_lasso.intercept_)
